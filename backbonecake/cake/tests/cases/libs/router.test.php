 ...
 */
	function testParsingWithPatternOnAction() {
		...
		$result = Router::parse('/album/other');
		$expected = array(
			'plugin' => null,
		...
		$result = Router::parse('/album/foobar');
		$expected = array(
			'plugin' => null,
			'controller' => 'album',
			'action' => 'foobar',
		...
		$this->assertEqual('/album/actions', $result);
	}
 ...
 **/
	function testMatchBasic() {
		...
		$route =& new CakeRoute('/album/:action', array('controller' => 'posts'));
		$result = $route->match(array('controller' => 'posts', 'action' => 'view'));
		$this->assertEqual($result, '/album/view');
