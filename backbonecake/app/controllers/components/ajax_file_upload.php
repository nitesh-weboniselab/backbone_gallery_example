...
class AjaxFileUploadComponent extends Object
{
    ...
    function uploadFiles($folder, $permitted, $sizeLimit = null, $replaceOldFile= FALSE, $height= 1, $width= 1)
    {
        ...
        if($height > 1 && $width > 1){
            if(!($this->check_height_and_width($this->file->getTmpPath(), array('width' => $width, 'height' => $height), 'greater'))){
                return json_encode(array('error' => "Please upload file in given extension  $width X $height."));
            }
        }
...
//                $filename .= rand(10, 99);
        ...
        if ($this->file->save($folder_url . DIRECTORY_SEPARATOR . $filename . $ext)){
            return json_encode(array('success' => array(true), 'filename' => array($filename . $ext),'error'=>''));
        } else {
    ...
    public function check_height_and_width($value, $allowed_height_width, $comparison_type)
    {
        ...
        switch ($comparison_type) {
            case 'equal':
                ...
                if ($width == $allowed_height_width['width'] && $height == $allowed_height_width['height']) {
                    return true;
                } else {
                    return false;
                }
